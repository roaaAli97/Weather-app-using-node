const request=require('request')


const geocode=(address,callback)=>{
  const url=`https://api.mapbox.com/geocoding/v5/mapbox.places/${address}.json?access_token=pk.eyJ1Ijoicm9hYWFsaSIsImEiOiJja3ZsNGhnbzU5MnN6Mm5zN24wbXdyNWVmIn0.5x4eI0w-ozkPhsCoIrU4xg&limit=1`
  request({url:url,json:true},(error,{body}={})=>{
       console.log(body)
      if(error){
          callback("Unable to connect to weather service.Check your internet connection",undefined)
      }
      else if(body.features.length===0){
          callback("Unable to find this location",undefined)
      }
      else{
          callback(undefined,{latitude:body.features[0].center[1],
                     longitude:body.features[0].center[0],
       
                     location:body.features[0].place_name})
                     
      }
  })
}
module.exports=geocode